...
// Copyright (c) 2013 Altcoin Developers
...
// Altcoin: Return average network hashes per second based on last number of blocks.
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Altcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Altcoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // Altcoin: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
...
void ThreadRPCServer2(void* parg)
{
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 9999));
